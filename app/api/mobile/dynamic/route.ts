...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. Fetch Wallets from user_preferences or fallback default
    ...
    if (prefWallets?.value) {
      try {
        wallets = JSON.parse(prefWallets.value);
      } catch (e) {}
    }
    ...
    let goals = [
      ...
      { id: 'goal-beat', title: 'Sinking Fund Ganti Ban & Servis Beat', target_amount: 450000, current_amount: 150000, target_date: '2026-09-15', status: 'in_progress', icon: 'two_wheeler' },
      { id: 'goal-pajak', title: 'Sinking Fund Pajak STNK Tahunan', target_amount: 250000, current_amount: 62500, target_date: '2026-11-20', status: 'in_progress', icon: 'receipt_long' }
    ];

    if (prefGoals?.value) {
      try {
        goals = JSON.parse(prefGoals.value);
      } catch (e) {}
    }
    ...
    if (prefBills?.value) {
      try {
        bills = JSON.parse(prefBills.value);
      } catch (e) {}
    }
    ...
    let pills = [
      { id: 'p-line', label: 'Line Chart Tren Kas', query: 'buatkan line chart tren arus kas harian dan burn rate', icon: 'show_chart', color: 'text-tosca' },
      { id: 'p-donut', label: 'Donut Chart Kategori', query: 'buatkan donut chart alokasi pengeluaran per kategori', icon: 'pie_chart', color: 'text-amber' },
      { id: 'p-bar', label: 'Bar Chart Keuangan', query: 'buatkan chart analisis perbandingan pemasukan vs pengeluaran', icon: 'bar_chart', color: 'text-lime' },
      { id: 'p-gantt', label: 'Gantt Chart 2026', query: 'tampilkan visual gantt chart roadmap 2026', icon: 'calendar_month', color: 'text-primary' },
      { id: 'p-map', label: 'Peta & Rute Dieng', query: 'tampilkan rute dan peta lokasi trip ke Dieng', icon: 'map', color: 'text-emerald' },
      { id: 'p-bensin', label: 'Bensin Beat 20k', query: 'saya isi bensin beat 20000 pakai cash kertas', icon: 'local_gas_station', color: 'text-amber' },
      { id: 'p-gojek', label: 'Narik Gojek 85k', query: 'pemasukan narik gojek hari ini 85000 masuk gopay driver', icon: 'two_wheeler', color: 'text-lime' }
    ];

    if (prefPills?.value) {
      try {
        pills = JSON.parse(prefPills.value);
      } catch (e) {}
    }
